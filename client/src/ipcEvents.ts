export const ipcEvents = {
    REGISTER: 'REGISTER',
    PRUNE_TOKENS: 'PRUNE_TOKENS',
    GET_AUTH_INFO: 'GET_AUTH_INFO',
    GET_ACCOUNTS: 'GET_ACCOUNTS',
    SET_ACCOUNT: 'SET_ACCOUNT',
    GET_ACCOUNT: 'GET_ACCOUNT',
    GET_SHARES: 'GET_SHARES',
    GET_CANDLES: 'GET_CANDLES',
    SUBSCRIBE_ORDER: 'SUBSCRIBE_ORDER',
    SUBSCRIBE_CANDLES: 'SUBSCRIBE_CANDLES',
    UNSUBSCRIBE_CANDLES: 'UNSUBSCRIBE_CANDLES',
    NEW_CANDLE: 'NEW_CANDLE',
    NEW_ORDER: 'NEW_ORDER',
    GET_SHARES_FROM_STORE: 'GET_SHARES_FROM_STORE',
    START_TRADE: 'START_TRADE',
    STOP_TRADE: 'STOP_TRADE',
    IS_STARTED: 'IS_STARTED',
    GET_TRADING_SCHEDULES: 'GET_TRADING_SCHEDULES',
    GET_STRATEGY_CONFIG: 'GET_STRATEGY_CONFIG',
    CHANGE_STRATEGY_CONFIG: 'CHANGE_STRATEGY_CONFIG',
    SUBSCRIBE_STRATEGY_ACTIVITIES: 'SUBSCRIBE_STRATEGY_ACTIVITIES',
    NEW_STRATEGY_ACTIVITY: 'NEW_STRATEGY_ACTIVITY',
    RESIZE: 'RESIZE',
    GET_CURRENT_INSTRUMENT: 'GET_CURRENT_INSTRUMENT',
    SET_CURRENT_INSTRUMENT: 'SET_CURRENT_INSTRUMENT',
} as const;